                            ...
                            <div class="card-body">
                                <div class="tab-content" id="myTabContent2">
                                    ...
                                    <div class="tab-pane fade" id="email_template_tab" role="tabpanel">
                                        @php
                                            // Friendly labels for the new notification email templates.
                                            $notifRegistry = \App\Notifications\NotificationEmailTemplates::all();
                                            // Split templates into the legacy bucket and the new in-app-notification bucket
                                            $systemTpls = collect($templates)->filter(fn ($t) => !str_starts_with($t->name, 'notif_'))->values();
                                            $notifTpls  = collect($templates)->filter(fn ($t) => str_starts_with($t->name, 'notif_'))->values();
                                            $prettyName = function ($name) use ($notifRegistry) {
                                                if (isset($notifRegistry[$name]['label'])) return $notifRegistry[$name]['label'];
                                                return ucfirst(\Str::replace('_', ' ', $name));
                                            };
                                        @endphp

                                        {{-- ===== System / transactional ===== --}}
                                        <h5 style="margin:0 0 8px; color:#1c1a4a; font-weight:600;">
                                            <i class="fas fa-cog text-muted"></i> {{ __('System emails') }}
                                        </h5>
                                        <p class="text-muted" style="font-size:13px; margin-bottom:12px;">
                                            {{ __('Transactional emails sent for password resets, registration, contact forms etc.') }}
                                        </p>
                                        <div class="table-responsive table-invoice mb-4">
                                        ...
                                        {{-- ===== Notification emails (admin-editable) ===== --}}
                                        @if ($notifTpls->count() > 0)
                                            <h5 style="margin:18px 0 8px; color:#1c1a4a; font-weight:600;">
                                                <i class="fas fa-bell text-primary"></i> {{ __('Notification emails') }}
                                                <span class="badge badge-primary" style="font-size:11px; margin-left:6px;">{{ $notifTpls->count() }}</span>
                                            </h5>
                                            <p class="text-muted" style="font-size:13px; margin-bottom:12px;">
                                                {{ __('Emails sent automatically alongside in-app bell notifications. Edit the subject/body and use the listed variables to customise each event.') }}
                                            </p>
                                            <div class="table-responsive table-invoice">
                                                <table class="table table-striped">
                                                    ...
                                                    <tbody>
                                                        @foreach ($notifTpls as $index => $item)
                                                            <tr>
                                                                <td>{{ $index + 1 }}</td>
                                                                <td>
                                                                    <strong>{{ $prettyName($item->name) }}</strong>
