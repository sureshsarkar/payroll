<div class="tab-pane fade active show" id="general_tab" role="tabpanel">
    <form action="{{ route('admin.update-general-setting') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input type="text" name="years_of_exprience" class="form-control" value="{{ $setting?->years_of_exprience }}">
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    <label for="">{{ __('Satisfied clients') }}</label>
                    <input type="text" name="satisfied_clients" class="form-control" value="{{ $setting?->satisfied_clients }}">
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    <label for="">{{ __('Countries Reached') }}</label>
                    <input type="text" name="countries_reached" class="form-control" value="{{ $setting?->countries_reached }}">
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    <label for="">{{ __('Classes Conducted') }}</label>
                    <input type="text" name="classes_conducted" class="form-control" value="{{ $setting?->classes_conducted }}">
                </div>
            </div>
        </div>
        <div class="form-group">
        ...
        {{-- 2026-05-29 UI/UX audit P1-3: sticky form actions bar.
             General settings is a 30+ field form; the submit button was
             below the fold. Sticky pin keeps it visible while operator
             scrolls through the form. --}}
        <div class="cs-form-actions cs-form-actions--sticky cs-form-actions--mobile-stack">
            <button class="btn btn-primary cs-form-actions__primary" type="submit">{{ __('Update') }}</button>
        </div>
