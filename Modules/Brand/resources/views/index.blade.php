        ...
        <section class="section">
            <div class="section-header">
                ...
                <div class="section-header-breadcrumb">
                    ...
                    <div class="breadcrumb-item">{{ __('Brand List') }}</div>
                </div>
            </div>
                                ...
                                <form method="GET" class="mb-3" role="search">
                                    <div class="input-group" style="max-width:360px;">
                                        <input type="text" name="search" value="{{ request('search') }}" class="form-control"
                                            placeholder="{{ __('Search name...') }}" aria-label="{{ __('Search brands') }}">
                                    ...
                                    <table class="table table-striped">
                                        ...
                                        <tbody>
                                            @forelse ($brands as $brand)
                                                <tr>
                                                    <td>{{ $loop->index + 1 }}</td>
                                                    {{-- <td class="bg-transparent-black"><img src="{{ asset($brand->image) }}" height="50px" width="50px" alt=""></td> --}}
                                                    <td>{{ $brand->name }}</td>
                                                    <td>
                                                        <input onchange="changeStatus({{ $brand->id }})"
                                            ...
                                            @empty
                                                <x-empty-table :name="__('Brand')" route="admin.brand.create"
...
@push('js')
    <script>
        function deleteData(id) {
            $("#deleteForm").attr("action", '{{ url('/admin/brand/') }}' + "/" + id)
        }
        ...
        function changeStatus(id) {
            ...
            $.ajax({
                ...
                url: "{{ url('/admin/brand/status-update') }}" + "/" + id,
