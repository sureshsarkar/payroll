...
class TestimonialController extends Controller
{
    ...
    public function index()
    {
        ...
        // 2026-06-02 (CRUD audit) — search by reviewer name / comment across
        // all translations (text lives in testimonial_translations).
        $search = trim((string) request('search'));
        $testimonials = Testimonial::with('translation')
            ->when($search !== '', fn ($q) => $q->whereHas('translations', fn ($t) => $t
                ->where('name', 'like', "%{$search}%")->orWhere('comment', 'like', "%{$search}%")))
