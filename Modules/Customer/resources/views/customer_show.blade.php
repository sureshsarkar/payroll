...
@section('admin-content')
                    ...
                    <div class="col-lg-3">
                        <div class="card shadow">
                            ...
                            <div class="container my-3">
                                ...
                                <a href="javascript:;" data-toggle="modal" data-target="#subscriptionModal"
                                    class="btn btn-success my-2"><i class="fas fa-hand-holding-usd"></i>
                                    {{ __('Get Subscription') }}</a>

                                {{-- Coach-specific payment gateway config (2026-06-29) --}}
                                <a href="{{ route('admin.coach-payment-gateways.index', $user->id) }}"
                                    class="btn btn-info my-2 text-white"><i class="fas fa-credit-card"></i>
                                    {{ __('Payment Gateway') }}</a>
                                                ...
                                                <div class="dashboard__review-table table-responsive">
                                                    <table class="table table-borderless">
                                                        ...
                                                        <tbody>
                                                            ...
                                                            @empty
                                                                {{-- 2026-05-29 UI/UX audit P1-11: friendly empty state. --}}
                                                                <tr>
                                                                    <td colspan="6" class="py-5 text-center text-muted">
                                                                        <i class="fas fa-inbox" style="font-size:32px; opacity:0.4;"></i>
                                                                        <div class="mt-2">{{ __('No data to display.') }}</div>
                                                                    </td>
                                                                </tr>
                                                            @endforelse
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form action="{{ route('admin.customer-Social-update', $user->id) }}" method="POST"
                                    ...
                                    <div class="form-group">
                                        <label for="insatagram">{{ __('Instagram') }}</label>
                                        <input id="insatagram" name="instagram" type="url"
                                            value="{{ $user->instagram }}" class="form-control">
                                    </div>

                                    <div class="form-group">
                                        <label for="youtube">{{ __('yt') }}</label>
                                        <input id="youtube" name="youtube" type="url"
                                            value="{{ $user->youtube }}" class="form-control">
                                    </div>
    ...
    <!-- Start Banned modal -->
    <div class="modal fade" id="subscriptionModal" tabindex="-1" role="dialog" aria-labelledby="modelTitleId"
        aria-hidden="true">
        <div class="modal-dialog" style="max-width: 1000px" role="document">
            <div class="modal-content card-bg-color">
                <div class="modal-body">
                    <div class="container-fluid">
                        <div class="custom-modal-header">
                            <h5 class="modal-title">{{ __('Get Subscription For This Instructor') }}</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form action="{{ route('admin.assign-subscription', $user->id) }}" method="POST">
                            @csrf
                                ...
                                <div class="row g-4 mt-3">
                                    ...
                                    @foreach ($plans as $plan)
                                        <div class="col-md-4 my-2">
                                            <div class="card plan-card h-100 shadow-sm text-center"
                                                data-plan-id="{{ $plan->id }}">
                                                <div class="card-body">
                                                    <h5 class="card-title">{{ $plan->name }}</h5>
                                                    <h2 class="text-primary fw-bold">₹{{ $plan->price }}</h2>
                                                    <h5 class="text-muted">{{ $plan->duration_days }} Days</h5>
                                                    <p class="text-muted lh-1">{{ $plan->description }}</p> 
                                                    <p class="btn btn-outline-primary w-100"> Choose Plan  </p>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn btn-primary" type="submit">{{ __('Confirm Subscription') }}</button>
                    </form>
                </div>
            </div>
        </div>
        ...
        document.querySelectorAll('.plan-card').forEach(card => {
            card.addEventListener('click', function() {
                ...
                document.getElementById('selectedPlan').value =
                    this.getAttribute('data-plan-id');
            });
        });
