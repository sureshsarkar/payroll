...
trait SyncModelsTrait
{
    ...
    private function createModels($defaultCode, $newCode)
    {
        ...
        foreach ($models as $translateAbleModel) {
            ...
            // 2026-05-26 (bug-doc A4) — wrap PER MODEL so one module with
            // a broken translation table (e.g. missing column from a stale
            // migration) doesn't abort the whole language create. The
            // outer language row already saved.
            try {
                ...
                Log::warning('language-sync-model-skip', [
                    'model'    => $translateAbleModel,
                    'new_code' => $newCode,
                    'error'    => $e->getMessage(),
                ]);
                continue;
