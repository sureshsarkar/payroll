...
class MenubuilderController extends Controller
{
    ...
    private function safeLinkOrFail(?string $url): string
    {
        $url = is_string($url) ? trim($url) : '';
        // Block javascript:, vbscript:, data: pseudo-URLs (XSS via href)
        if ($url !== '' && preg_match('#^\s*(javascript|vbscript|data)\s*:#i', $url)) {
            abort(422, 'Disallowed URL scheme');
        }
        return $url;
    }
        ...
        if (is_array($arrayData)) {
            foreach ($arrayData as $value) {
                ...
                if($code == config('app.locale')){
                    ...
                    $menuItem->link = $this->safeLinkOrFail($value['link'] ?? '');
                    if (config('menubuilder.use_roles')) {
            ...
            if(request()->input("code") == config('app.locale')){
                ...
                $menuItem->link = $this->safeLinkOrFail(request()->input("url"));
    ...
    {
        // FT-IDOR-15 fix (2026-05-28) — addMenuItem was the only
        // public method in this controller without the
        // `menu.update` gate that every other mutator uses
        // (updateMenu / updateMenuItem / deleteMenu / deleteMenuItem).
        // Without it a sub-admin without menu.update could append
        // arbitrary menu items to ANY of the platform's menus.
        checkAdminHasPermissionAndThrowException('menu.update');
