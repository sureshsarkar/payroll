...
class WithdrawApprovalJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        // F51 (audit 2026-06-26) — guard the null template: previously
        // $template->subject threw an NPE outside the try/catch, dumping the job
        // into failed_jobs with no recovery if the template was missing/renamed.
        if (! $template) {
            logger()->warning("WithdrawApprovalJob: email template '{$this->status}_withdraw' not found; skipping mail.");
            return;
        }
    ...
    /** F51 — surface a hard failure instead of a silent failed_jobs row. */
    public function failed(\Throwable $e): void
    {
        logger()->error('WithdrawApprovalJob failed: ' . $e->getMessage());
    }
}
