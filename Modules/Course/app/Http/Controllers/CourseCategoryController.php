...
class CourseCategoryController extends Controller
{
    ...
    {
        // FT-IDOR-5 fix (2026-05-27) — no permission gate.
        // All four course-taxonomy controllers (Category, SubCategory,
        // Language, Level) sit behind `auth:admin` but the controllers
        // themselves had no `checkAdminHasPermission*` call on most
        // methods. Use `course.management` to match the convention in
        // CourseController / CourseContentController / CourseDeleteRequestController.
        // PermissionsTrait::$coursePermissions registers exactly one
        // permission (`course.management`) so there is no finer-grained
        // key to use today.
    ...
    {
        // FT-IDOR-5 (typo fix) — previously gated on `blog.category.update`,
        // a clear copy-paste from BlogCategoryController. Effect: an admin
        // with blog-only permissions could toggle COURSE categories on/off,
        // and a course admin without blog permissions could not toggle even
        // their own course categories. Corrected to `course.management`
        // (the only registered course permission; see PermissionsTrait).
        checkAdminHasPermissionAndThrowException('course.management');
