...
class ChapterLessonRequest extends FormRequest {
    ...
    function liveRules(): array {
        ...
        if ($this->live_type == 'zoom') {
            // Passcode is no longer required (or stored) — the LMS-side
            // signature endpoint authenticates joiners, and meetings are
            // created with `password: false` settings on Zoom (see
            // LiveClassController::store/update). join_url is still
            // accepted for legacy compatibility but optional.
