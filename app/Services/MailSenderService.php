        ...
        if (self::setMailConfig()) {
            try {
                if (self::isQueable()) {
                    ...
                    if ($user_type == 'all_user') {
                        // Chunk to bound memory usage — was loading the full active-verified
                        // users list at once which OOMs at scale.
                        User::where(['status' => 'active', 'is_banned' => 'no'])
                            ->whereNotNull('email_verified_at')
                            ->orderBy('id', 'desc')
                            ->chunkById(500, function ($users) use ($mail_message, $mail_subject) {
                                foreach ($users as $user) {
                                    try {
                                        Mail::to($user->email)->send(new SendMailToUser($mail_message, $mail_subject));
                                    } catch (Exception $ex) {
                                        if (app()->isLocal()) Log::error($ex->getMessage());
                                    }
                                }
                            });
                    } else {
    ...
    public function sendLiveClassNotificationMailTrait($users, $data) {
        if (self::setMailConfig()) {
            try {
                ...
                foreach ($users as $user) {
                    $email = $user?->email;
                    // 2026-07-09 fix (Phase 1 / 1.1): personalise into a per-recipient
                    // copy. Previously str_replace ran on the shared $message inside the
                    // loop, so once {{user_name}} was consumed on iteration 1 every later
                    // student's email was addressed to the FIRST student's name.
                    $personalized = str_replace('{{user_name}}', $user?->name, $message);
                    $personalized = strip_unresolved_tokens($personalized); // Phase 1.7
                    if (self::isQueable()) {
                        dispatch(new sendLiveClassMailJob($email, $subject, $personalized));
                    } else {
                        Mail::to($email)->send(new sendLiveClassMail($subject, $personalized));
                    }
                }
                return true;
            } catch (Exception $th) {
