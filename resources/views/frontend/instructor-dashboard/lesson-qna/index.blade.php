...
@section('dashboard-contents')
    {{-- Lesson Questions. 2026-07-04: corp header + filter bar added; the Q&A
         thread cards (.wsus_*) and all JS hooks (instructor-qna.js: .question-view/
         markAsReadUnread, .delete-item, .replay-form, toggleEditor, .text-editor-*)
         are preserved verbatim below. --}}
    @include('frontend.instructor-dashboard.settings.partials._corporate')
    <div class="corp-page" id="lessonQna">
        <div class="corp-header">
            <div class="corp-header__title">
                <h4><i class="fas fa-comments" style="color:var(--corp-brand);"></i> {{ __('Lesson Questions') }}</h4>
                <p>{{ __('Answer the questions students left on your lessons.') }}</p>
            </div>
        </div>

        <div class="corp-form-card">
            <div class="corp-form-card__body" style="padding:14px 16px;">
                <form class="instructor__profile-form" style="display:flex;gap:10px;justify-content:flex-end;flex-wrap:wrap;margin:0;"
                    action="{{ route('instructor.lesson-questions.index') }}" method="GET" onchange="$(this).trigger('submit')">
                    <select name="seen" id="seen" class="form-select corp-select" style="max-width:170px;height:38px;">
                        <option value="">{{ __('All') }}</option>
                        <option value="1" {{ request('seen') == '1' ? 'selected' : '' }}>{{ __('Read') }}</option>
                        <option value="0"{{ request('seen') == '0' ? 'selected' : '' }}>{{ __('Unread') }}</option>
                    </select>
                    <select name="sort_by" id="sort_by" class="form-select corp-select" style="max-width:170px;height:38px;">
                        <option value="0" {{ request('sort_by') == '0' ? 'selected' : '' }}>{{ __('Newest ') }}</option>
                        <option value="1" {{ request('sort_by') == '1' ? 'selected' : '' }}>{{ __('Oldest ') }}</option>
                    </select>
                </form>
            </div>
        </div>
